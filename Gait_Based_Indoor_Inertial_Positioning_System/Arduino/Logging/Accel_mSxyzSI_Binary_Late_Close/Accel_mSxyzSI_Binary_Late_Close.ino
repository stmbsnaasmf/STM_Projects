...
#define CLOSING_MILLIS 500
uint32_t previousClosingMillis = 0;
File accelLogs;


struct Data
{
  uint8_t periodMillis;
  float ax, ay, az;
};
...
void setup()
{
  ...
  accelLogs = SD.open(LOG_FILE_NAME, O_CREAT | O_WRITE | O_APPEND);
  ...
  if (accelLogs)
  {
    ...
    toBeLogged.periodMillis = 0;
    toBeLogged.ax = 0;
    toBeLogged.ay = 0;
    toBeLogged.az = 0;

    //bFile.write( (const uint8_t *)&LogData, sizeof(LogData) );
    ...
    //accelLogs.close();    //Do NOT close. The loop requires the file to be already open.
...
void loop()
{
  ...
  if (millis() - previousClosingMillis >= CLOSING_MILLIS)
  {
    accelLogs.close();
  }
  ...
  if (millis() - previousClosingMillis >= CLOSING_MILLIS)
  {
    accelLogs = SD.open(LOG_FILE_NAME, O_WRITE | O_APPEND);
    previousClosingMillis = millis();
  }
  ...
  if (millis() - previousLoggingMillis >= loggingIntervalMS)
  {
    ...
    toBeLogged.periodMillis = uint8_t(millis() - previousLoggingMillis);
    ...
    {
      Serial.println("Could not open file in loop.");
