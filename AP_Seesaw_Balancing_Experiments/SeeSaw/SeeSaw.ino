...
//float roll;
int32_t rollError;
int32_t desiredRoll = 0;
uint32_t proportionalConstant = 280;  //Units in Hundreds
...
void loop()
{
  ...
  m1Position = USUAL_ESC_POSITION + 50;
  ...
  if (millis() - previousMillis >= 20)
  {
    ...
    rollError = (desiredRoll - currentRoll) * (-1);  //@0 desired, right side error would be 0 - (>0 && < 180) = 0 < rollError < 180
    Serial.print("Roll Error: "); Serial.println(rollError);
  }

  if (rollError >= 0 && rollError <= 18000) //Right side down, increase right(m1)'s thrust.
  {
    m1Position += (rollError * proportionalConstant) / 10000; //From 100 * 100
  }

  if (rollError > 18000 || rollError < 0 )  //Left side down
  {
    rollError = 36000 - currentRoll;
    Serial.print("Roll Error 2: "); Serial.println(rollError);
    m2Position += (rollError * proportionalConstant) / 10000;
  }
