...
uint32_t proportionalConstant = 290;  //Units in Hundreds
...
uint16_t integralConstant = 30000;    //From 100 to 1000 as a rough guide.
...
void loop()
{
  ...
  if (rollError >  -150 && rollError < 150)   //+-3 degrees
  {
    sumOfErrors = 0;
  }
  ...
  if (rollError > 0) //Right side down, increase right(m1)'s thrust.
  {
    ...
    m1Position += 0 + ((rollError * proportionalConstant) / 10000) + (sumOfErrors / integralConstant); //From 100 * 100   //P + I
  }
